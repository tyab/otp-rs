...
| `otp-fares` | 運賃計算（GTFS-Fares v1、将来 v2 / JR 距離制） | `ext.fares` | GTFS-Fares v1 (route/origin/destination/contains の一致規則、フィード内通し運賃) 実装済み・本家 OTP と数値突き合わせ済み・JR 距離制は TODO |
| `otp-engine` | 統合（アクセス徒歩→乗換→イグレス徒歩→運賃→結果） | `routing.algorithm` 統合層 | 座標to座標の `Engine::plan` 実装済み（アクセス/イグレス徒歩+RAPTOR統合+フィード横断の運賃合算）・バス運賃/otp-server API化は次段 |
| `otp-server` | ネイティブ経路サーバ（JVM OTP を置換） | `standalone`, GraphQL API | 起動時ロード + `GET /health` / `POST /plan` 実装済み・起動時間を実測（下記） |
