...
fn prepare_feed_dir(feed_prefix: &str, zip_path: &Path) -> PathBuf {
    if !zip_path.is_file() {
        eprintln!(
            "GTFS zip が見つかりません: {} (feed={feed_prefix})",
            zip_path.display()
        );
        std::process::exit(1);
    }
...
fn load_all_feeds() -> Vec<Feed> {
    ...
    FEEDS
        ...
        .map(|(prefix, filename)| {
            ...
            Feed::load_from_dir_namespaced(&feed_dir, prefix)
                .unwrap_or_else(|e| panic!("feed {prefix} load failed: {e}"))
        })
        .collect()
}
...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!(
            "  stop_id はフィードID前置 (例: 6:428 = 都営 新宿, 3:805 = メトロ 六本木一丁目)"
        );
        std::process::exit(1);
    }
    ...
    let origin = tt
        .stop_idx(&otp_core::StopId::new(origin_id.as_str()))
        .unwrap_or_else(|| panic!("origin stop {origin_id} not found"));
    let dest = tt
        .stop_idx(&otp_core::StopId::new(dest_id.as_str()))
        .unwrap_or_else(|| panic!("dest stop {dest_id} not found"));
    ...
    for j in &journeys {
        ...
        println!(
            "arrival={h:02}:{m:02}:{s:02} ({}s) transfers={}",
            j.arrival_s, j.transfers
        );
        for leg in &j.legs {
            match leg {
                otp_raptor::JourneyLeg::Walk { stop, duration_s } => {
                    println!("  WALK stop_idx={stop} duration={duration_s}s")
                }
