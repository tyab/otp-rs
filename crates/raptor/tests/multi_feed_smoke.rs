...
fn cross_operator_transfer_at_shirokanetakanawa_is_found() {
    let Some(toei_dir) = prepare_feed_dir("6", "toei-train-gtfs.zip") else {
        return;
    };
    let Some(metro_dir) = prepare_feed_dir("3", "tokyometro-train-gtfs.zip") else {
        return;
    };

    let toei = Feed::load_from_dir_namespaced(&toei_dir, "6").expect("都営フィードのロードに失敗");
    let metro =
        Feed::load_from_dir_namespaced(&metro_dir, "3").expect("メトロフィードのロードに失敗");
    let tt = Timetable::build(&[toei, metro]).expect("timetable should build");

    // 六本木一丁目 (メトロ南北線, 3:805) → 三田 (都営三田線, 6:204)。
    let roppongi_itchome = tt
        .stop_idx(&otp_core::StopId::new("3:805"))
        .expect("六本木一丁目 (3:805) が時刻表に無い");
    let mita = tt
        .stop_idx(&otp_core::StopId::new("6:204"))
        .expect("三田 (6:204) が時刻表に無い");

    let query = RaptorQuery {
        access: vec![StreetLink {
            stop: roppongi_itchome,
            duration_s: 0,
        }],
    ...
    assert!(
        !journeys.is_empty(),
        "六本木一丁目→三田 の経路が1つも見つからなかった"
    );

    let best = journeys.last().expect("経路が見つからなかった");
    assert_eq!(best.transfers, 1, "白金高輪で1回乗換のはず: {best:?}");

    let walk_legs: Vec<_> = best
        .legs
        .iter()
        .filter_map(|l| match l {
            JourneyLeg::Walk { duration_s, .. } => Some(*duration_s),
            _ => None,
        })
        .collect();
    assert_eq!(
    ...
    assert_eq!(
        transit_legs.len(),
        2,
        "南北線→三田線の2本乗り継ぎのはず: {transit_legs:?}"
    );
    assert!(
        transit_legs[0].contains("南北線"),
        "1本目は南北線のはず: {transit_legs:?}"
    );
    assert!(
        transit_legs[1].contains("三田線"),
        "2本目は三田線のはず: {transit_legs:?}"
    );
...
fn namespacing_prevents_service_id_collision_across_feeds() {
    ...
    let toei_services: HashSet<&str> = toei
        .calendars
        .iter()
        .map(|c| c.service_id.as_str())
        .collect();
    let metro_services: HashSet<&str> = metro
        .calendars
        .iter()
        .map(|c| c.service_id.as_str())
        .collect();
    ...
    assert!(
        toei_services.is_disjoint(&metro_services),
        "名前空間化後は service_id が事業者間で衝突しないはず"
    );
    ...
    assert!(
        toei_routes.is_disjoint(&metro_routes),
        "名前空間化後は route_id が事業者間で衝突しないはず"
    );
}
